...
#         pricelist = pd.read_excel(uploaded_pricelist, sheet_name=None)
...
def df_stats(df, df_p, df_s):
    ...
    grouped_df_pt = df_p.groupby(["Product Description"]).agg({"Amount": "sum"}).sort_values("Amount", ascending=False)
    grouped_df_final_pt = grouped_df_pt[['Amount']].head(10)
    st.table(grouped_df_final_pt.style.format({'Amount': '{:,.2f}'}))
    st.write('')
    st.write('**Top 10 stores by amount:**')
    grouped_df_st = df_s.groupby("Retailer").agg({"Amount": "sum"}).sort_values("Amount", ascending=False)
    grouped_df_final_st = grouped_df_st[['Amount']].head(10)
    st.table(grouped_df_final_st.style.format({'Amount': '{:,.2f}'}))
    st.write('')
    st.write('**Final Dataframe:**')
    st.dataframe(df)
...
if option == "Weekly Report":
    ...
    if submit_button and uploaded_file and uploaded_pricelist:
        ...
        final_df_p = final_df[['365 Code', 'Product Description', 'Amount']]
        final_df_s = final_df[['Retailer', 'Amount']]
